...
// ✅ Injecte le filtre "uniquement mes batchs" sur n'importe quel WHERE
...
function netExpr(sign) {
  return sign === 'positive' ? 'SUM(credit) - SUM(debit)' : 'SUM(debit) - SUM(credit)';
}
...
async function getDashboard(req, res) {
  try {
    ...
    if (myBatchIds.length === 0) {
      return res.json({
        periode: { year, month: monthIndex + 1, mode: isGlobal ? 'global' : 'month' },
        caMois: 0,
        caMoisPrec: 0,
        encaissements: 0,
        decaissements: 0,
        ...
        echeances: {
          clients: {
            retard: 0, a7j: 0, a30j: 0,
            nbRetard: 0, nbA7j: 0, nbA30j: 0,
            retardTrend: { delta: 0, pct: 0, direction: 'up' },
            count: 0, oldestDate: null, top: [],
            details: { retard: [], a7j: [], a30j: [] },
          },
          fournisseurs: {
            retard: 0, a7j: 0, a30j: 0,
            nbRetard: 0, nbA7j: 0, nbA30j: 0,
        ...
        alerts: [
          { level: 'green', code: 'NO_DATA', title: 'Aucune donnée', message: 'Aucun import trouvé pour cet utilisateur.' },
        ],
      });
    }
    ...
    const refDate = isGlobal
      ? now
      : (isCurrentMonth ? now : new Date(year, monthIndex + 1, 0, 23, 59, 59, 999));
    ...
    const soldeOuverture = await getSoldeOuvertureForYear(openingYear, myBatchIds);
    ...
    const [caMois, caMoisPrec, chargesMois, sumCaYTD, sumChargesYTD] = await Promise.all([
      ...
      isGlobal
        ? sumField(
            ...
            {
              date: {
                [Op.gte]: new Date(now.getTime() - 60 * 24 * 3600 * 1000),
                [Op.lt]: new Date(now.getTime() - 30 * 24 * 3600 * 1000),
              },
              ...caFilter,
            },
            myBatchIds
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      if (
        i === 0 &&
        (isGlobal || (year === now.getFullYear() && monthIndex === now.getMonth())) &&
        dEnd > asOf
      ) {
        dEnd = asOf;
      }
